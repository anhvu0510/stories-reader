...
export function TokenManagerSheet({ onClose, isEmbedded = false }: { onClose?: () => void; isEmbedded?: boolean }) {
  ...
  const fetchTokens = async () => {
    setIsLoading(true);
    try {
      const res = await AIRepository.getTokens(activeTab);
      if (res) {
        setTokens(res.tokens || []);
      }
    } catch {
      showToast('Lỗi khi tải danh sách token API', 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await AIRepository.deleteToken(id);
      showToast('Đã xoá token', 'success');
      fetchTokens();
    } catch {
      showToast('Lỗi khi xoá token', 'error');
    }
  };
  ...
  const handleToggleStatus = async (t: AIToken) => {
    try {
      ...
      await AIRepository.updateToken(t._id, { status: newStatus });
      showToast(`Đã ${newStatus === 'active' ? 'kích hoạt' : 'tạm dừng'} token`, 'success');
      fetchTokens();
    } catch {
      showToast('Lỗi cập nhật trạng thái token', 'error');
    }
  };

  const handleSave = async () => {
    if (!formData.name || !formData.email || (!formData.configAI && editingId === 'new')) {
      showToast('Vui lòng điền đủ Tên, Email và Cấu hình AI', 'error');
      return;
    }
    ...
    try {
      ...
      if (editingId === 'new') {
        await AIRepository.createToken(submissionData);
      } else {
        await AIRepository.updateToken(editingId!, submissionData);
      }
      showToast('Đã lưu token thành công', 'success');
      setEditingId(null);
      fetchTokens();
    } catch {
      showToast('Lỗi khi lưu token', 'error');
    }
  };
                  ...
                  <div className="flex justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {t.status === 'active' ? (
                          <span className="text-[9px] bg-primary/20 text-primary px-1.5 py-0.5 rounded font-medium shrink-0">
                            ACTIVE
                          </span>
                        ) : t.status === 'banned' ? (
                          <span className="text-[9px] bg-error/20 text-error px-1.5 py-0.5 rounded font-medium shrink-0">
                            BANNED
                          </span>
                        ) : (
                      ...
                      {t.status !== 'banned' && (
                        <button
                          ...
                          className={`p-2 rounded-lg transition-colors ${
                            t.status === 'active' ? 'text-warning hover:bg-warning/10' : 'text-primary hover:bg-primary/10'
                          }`}
                        >
                      ...
                      {t.modelList.map((m) => (
                        <div key={m.model} className="bg-surface-container flex items-center justify-between p-2 rounded-lg">
                          ...
                          <div className="flex gap-3 text-[9px] font-medium text-on-surface-variant/80">
                            {m.rpdLimit > 0 && (
                              <span>
                                RPD: {m.usageToday?.rpd || 0}/{m.rpdLimit} ({m.usageToday?.rpdPercent || 0}%)
                              </span>
                            )}
          ...
          {tokens.length === 0 && editingId !== 'new' && (
            <div className="text-center py-12 text-on-surface-variant">
              <p className="text-sm border border-dashed border-outline-variant/30 p-4 rounded-xl">
                Chưa có token nào được cấu hình cho tab này
              </p>
            </div>
          )}
        </div>
...
function TokenEditor({
  ...
  formData: Partial<AIToken>;
  setFormData: (d: Partial<AIToken>) => void;
  onSave: () => void;
  onCancel: () => void;
  isNew: boolean;
}) {
  const configAI =
    typeof formData.configAI === 'string'
      ? (() => {
          try {
            return JSON.parse(formData.configAI);
          } catch {
            return {};
          }
        })()
        ...
        {formData.platform === 'AI_STUDIO' ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, configAI: { ...configAI, vertexai: true, project: e.target.value } })
              }
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, configAI: { ...configAI, vertexai: true, location: e.target.value } })
              }
